...
public:
    ...
    // address=[0x1329020]
    static class CAITaskForce * __cdecl CreateTaskForce(int _iPlayerId, enum T_AI_TASK_FORCE_TYPE _iTaskForceType);
...
protected:
    ...
    // address=[0x3ECCD28]
    static struct STaskForcesData {
        int           m_iTotalNumberOfTaskForces;
        int           m_iTaskForceRemainder;
        CAITaskForce *m_pFirstTaskForce;
        CAITaskForce *m_pCurrentTaskForce;
    } s_sTaskForcesData;

    // address=[0x3ECCD24]
    static bool s_bTaskForcesInitialized;
};
