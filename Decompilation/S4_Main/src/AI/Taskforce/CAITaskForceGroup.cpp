...
void CAITaskForceGroup::DeleteAllTaskForces(void) {
  CAITaskForce *TaskForceGroupMemberOfSameClass; // [esp+0h] [ebp-20h]
  for(int tTaskForceClass = 0; tTaskForceClass < AI_TASK_FORCE_CLASS_MAX; ++tTaskForceClass) {
    ...
    BB_ASSERT(this->FirstTaskForce(static_cast<T_AI_TASK_FORCE_CLASS>(tTaskForceClass)) == nullptr);
...
void CAITaskForceGroup::Load(class IS4Chunk &a2) {
  ...
  if(uint32_t uTaskForceCount = a2.LoadUnsigned32(); uTaskForceCount > 0) {
    auto **pTaskForces = new CAITaskForce *[uTaskForceCount]; // NOTE: Again, they allocate an array, never delete it, and for really nothing...
    for(int i = 0; i < uTaskForceCount; ++i) {
      auto tTaskForceType = static_cast<T_AI_TASK_FORCE_TYPE>(a2.LoadUnsigned32(0, 13));
      a2.LoadUnsigned32();
      CAITaskForce *pTaskForce = this->CreateTaskForce(tTaskForceType);
      BB_ASSERT(pTaskForce != nullptr);
      // if(!pTaskForce && BBSupportDbgReport(2, "AI\\AI_TaskForces.cpp", 1012, "pTaskForce != 0") == 1)
      //   __debugbreak();
      pTaskForces[i] = pTaskForce;
      pTaskForce->Load(a2);
      uint32_t uAssociatesCount = a2.LoadUnsigned32();
      if(uAssociatesCount >= 0 && uAssociatesCount < i) {
        CAITaskForce *pAssociatedTaskForce = pTaskForces[uAssociatesCount];
        BB_ASSERT(pAssociatedTaskForce != nullptr);
...
void CAITaskForceGroup::Save(class IS4Chunk &a2) {
  ...
  a2.SaveUnsigned32(this->PlayerId());
  ...
  if(iNumberOfTaskForces > 0) {
    ...
    for(int k = 1; k < AI_TASK_FORCE_CLASS_MAX; ++k) {
      for(CAITaskForce *m = this->FirstTaskForce(, k); m; m = CAITaskForce::NextTaskForceGroupMemberOfSameClass(m)) {
        BB_ASSERT(iTaskForceCounter < iNumberOfTaskForces);
        ...
        //   __debugbreak();
        // }
        a2.SaveUnsigned32(m->Type());
        a2.SaveUnsigned32(0);
        m->Save(m, a2);
        int           v4 = -1;
        CAITaskForce *pAssociatedTaskForce = m->AssociatedTaskForce();
        if(pAssociatedTaskForce) {
          BB_ASSERT(pAssociatedTaskForce->TaskForceGroup() == this);
          ...
          if(pAssociatedTaskForce->TaskForceGroup() == this) {
            ...
            BB_ASSERT(iAssociatedId < iNumberOfTaskForces);
...
void CAITaskForceGroup::AddTaskForce(class CAITaskForce *_pTaskForce) {
  ...
  BB_ASSERT(_pTaskForce != nullptr);
  // if(!_pTaskForce && BBSupportDbgReport(2, "AI\\AI_TaskForces.cpp", 891, "_pTaskForce != 0") == 1)
  //   __debugbreak();
  BB_ASSERT(_pTaskForce->m_pTaskForceGroup == nullptr);
  ...
  BB_ASSERT(iTaskForceClass >= 0 && iTaskForceClass < AI_TASK_FORCE_CLASS_MAX);
  ...
  //   __debugbreak();
  // }
  _pTaskForce->m_pPrevTaskForceGroupMember = nullptr;
  ...
  if(this->m_sData.m_pTaskForces[iTaskForceClass]) {
    BB_ASSERT(this->m_sData.m_pTaskForces[iTaskForceClass]->m_pPrevTaskForceGroupMember == nullptr);
    ...
    BB_ASSERT(this->m_sData.m_iNumberOfTaskForcesOfClass[iTaskForceClass] == 0);
    // if(this->m_sData.m_iNumberOfTaskForcesOfClass[iTaskForceClass]
    //    && BBSupportDbgReport(
    //      2,
    //      "AI\\AI_TaskForces.cpp",
    //      911,
...
void CAITaskForceGroup::RemoveTaskForce(class CAITaskForce *_pTaskForce) {
  ...
  //   __debugbreak();
  BB_ASSERT(_pTaskForce->m_pTaskForceGroup == this);
  ...
  if(_pTaskForce->m_pPrevTaskForceGroupMember) {
    BB_ASSERT(_pTaskForce->m_pPrevTaskForceGroupMember->m_pNextTaskForceGroupMember == _pTaskForce);
    ...
    BB_ASSERT(this->m_sData.m_pTaskForces[iTaskForceClass] == _pTaskForce);
  ...
  if(_pTaskForce->m_pNextTaskForceGroupMember) {
    BB_ASSERT(_pTaskForce->m_pNextTaskForceGroupMember->m_pPrevTaskForceGroupMember == _pTaskForce);
  ...
  --this->m_sData.m_iNumberOfTaskForcesOfClass[iTaskForceClass];
  --this->m_sData.m_iNumberOfTaskForcesOfType[_pTaskForce->Type()];
  BB_ASSERT(this->m_sData.m_iTotalNumberOfTaskForces >= 0);
  ...
  //   __debugbreak();
  // }
  BB_ASSERT(this->m_sData.m_iNumberOfTaskForcesOfType[_pTaskForce->Type()] >= 0);
