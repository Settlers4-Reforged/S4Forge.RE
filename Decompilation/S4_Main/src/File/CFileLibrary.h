...
extern class CFileLibrary *g_cFileLibrary;
...
public:
    ...
    // address=[0x2f0b6c0]
    class std::vector<struct std::pair<std::wstring, std::wstring>> ListFilesInLib(wchar_t const *a2);
    ...
    // address=[0x2F0D610]
    static int __cdecl sDirCompare_qsort(struct FLMemDirStruct const *, struct FLMemDirStruct const *);

    // address=[0x2F0D630]
    static int __cdecl sFileCompare_qsort(struct FLMemFileStruct const *, struct FLMemFileStruct const *);
...
private:
    ...
    // address=[0x2F08EA0]
    FLMemDirStruct *pFindDirectory(wchar_t *path);

    // address=[0x2F08C10]
    FLMemFileStruct *pFindFile(wchar_t *_swpFileName, FLMemDirStruct *_pDir);

    // address=[0x2F08000]
    FLIntHandleStruct *pGetIntHandlePtr(unsigned int);

    void pMoveHandleToBeginning(struct FLFHandleStruct *);

    // address=[0x02F08CB0]
    FLMemDirStruct *pAddDirectory(wchar_t const *);

    // address=[0x02F088F0]
    void pAddFile(struct FLMemDirStruct *_curdir, const wchar_t *String, FLHeaderFileStruct *a3, __int16 a4, int a5);
...
public:
    ...
    FLIntHandleStruct *m_pFLIntHandle[100];
    ...
    int field_1D8;
    std::vector<std::pair<std::wstring, int>> *m_vFileIdPairs;
};
