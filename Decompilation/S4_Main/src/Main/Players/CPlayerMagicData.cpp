...
void CPlayerMagicData::MarkMaxSpellCastNumbersAsInvalid(void) {
  memset(this->m_aMaxSpellCastNumbers, -1, sizeof(this->m_aMaxSpellCastNumbers));
}
...
void CPlayerMagicData::Clear(void) {
  ...
  this->m_iFlags = 1;
  CPlayerMagicData::SetManaAmount(0);
  CPlayerMagicData::SetTotalAmountOfCollectedMana(0);
  CPlayerMagicData::SetMaxNumberOfBigTemples(0);
  for(i = 0; i < 8; ++i) {
    CPlayerMagicData::SetSpellCategoryCost(i, 0);
    CPlayerMagicData::SetSpellCategoryCounter(i, 0);
  }
  CPlayerMagicData::MarkMaxSpellCastNumbersAsInvalid();
}
