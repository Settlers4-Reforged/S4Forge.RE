...
int CFsm::Control(int _iEventId, void *_pEvent) {
    ...
    while(i) {
        ...
        if(hash < 0 || this->m_pTransitions[hash].m_iHandlerId == -1) {
            EventHandlerFunc handler = this->m_pEventHandler->m_pHandlers[0];
            handlerResult = (this->m_pEventHandler->*handler)(reinterpret_cast<void *>(i->m_iId));
        } else {
            EventHandlerFunc handler = this->m_pEventHandler->m_pHandlers[this->m_pTransitions[hash].m_iHandlerId];
            handlerResult = (this->m_pEventHandler->*handler)(i->m_pPayload);
