...
void IEntity::Store(std::ostream &outputFile) {
    ...
    outputFile << this->m_iEntityId;
    outputFile << this->m_iType;
    outputFile << this->m_uPackedXY;
    outputFile << this->m_cHealth;
    outputFile << this->m_uObjType;
...
void IEntity::Decrease(int byAmount) {
    ...
    if(byAmount > 0) {
        if(byAmount < this->m_cHealth) {
            ...
            this->m_cHealth = 0;
            g_pMapObjectMgr->Kill(this->m_iEntityId, g_CurrentEntityAttacker);
        }
    }
}
