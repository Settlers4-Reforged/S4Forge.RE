...
class CPile : public IAnimatedEntity {
  public:
    constexpr static int MAX_PILE_AMOUNT = 8;
    ...
    // address=[0x155c960]
    void CancelCompleteOfferIfInOfferList(int _iEcoSectorId);
  ...
  protected:
    ...
    // address=[0x155d720]
    bool ExecuteChangeGoodTypeUnforseen(int _iGood);

  private:
    friend class CPileMgr;

    // address=[0x155dae0]
    CPile(int _iX, int _iY, int _iGood, int _iAmount, class std::unique_ptr<class IPileRole> _pRole, int _iSlot, int a8, int a9, int _iOffsetX, int _iOffsetY);
  ...
  public:
    ...
    std::unique_ptr<IPileRole> m_pRole;
    CPileObserverList m_cPileObserverList;
};
