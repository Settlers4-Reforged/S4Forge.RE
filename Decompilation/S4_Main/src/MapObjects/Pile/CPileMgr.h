...
extern class CPileMgr g_cPileMgr;

class CPileMgr {
  public:
    constexpr static int MAX_PILE_COUNT = 20000;

    // address=[0x12fd1f0]
    static class CPile *__cdecl GetPilePtr(int _iPileId);

    // address=[0x1438a40]
    class CPile &operator[](int _iPileId);
    ...
    // address=[0x155e970]
    void LoadPileData(class S4::CMapFile &_rMap, int a3);
    ...
    // address=[0x155ede0]
    int GetNearestPile(int _iPileId, int _iX, int _iY);
  ...
  private:
    ...
    struct SPileData {
        unsigned __int16 m_iX;
        unsigned __int16 m_iY;
        unsigned __int8 m_iGood;
        signed __int8 m_iAmount;
        unsigned __int8 m_uPileType;
        __int8 pad;
    };
