...
bool CBuildingInfoMgr::DbgCheckBuildingBits(int _iRace, int _iBuildingType) {
  ...
  if(!pBuildingInfo->m_bIsPort
     && (pBuildingInfo->m_iBBRMinX < -15 || pBuildingInfo->m_iBBRMaY > 15 || pBuildingInfo->m_iBBRMinY < -15 || pBuildingInfo->m_iBBRMaxY > 15)) {
    v39 = 0;
    BBSupportTracePrintF(
      3,
  ...
  while(cBuildPosWalk.NextPosition()) {
    ...
    if(v5 < pBuildingInfo->m_iBBRMinX
       || (v6 = cBuildPosWalk.CurrentX(), v6 > pBuildingInfo->m_iBBRMaY)
...
void CBuildingInfoMgr::ReadBuildingInfo(void) {
  ...
  long  iXmlSize;
  auto *pXmlData = Parser::OpenXMLFile(L"GameData\\BuildingInfo.xml", iXmlSize);
  if(!pXmlData) {
  ...
  auto                   pDoc = std::unique_ptr<Document>(cParser.Parse(pXmlData, iXmlSize));
  ...
  while(rIt < rEnd) {
    ...
    while(rBuildingIt < rBuildingEnd) {
      ...
      rBuildingInfo.m_iHotSpotX = atoi(rBuildingElement("iHotSpotX", 0).GetValue().c_str());
      rBuildingInfo.m_iHotSpotY = atoi(rBuildingElement("iHotSpotY", 0).GetValue().c_str());
      rBuildingInfo.m_iStone = atoi(rBuildingElement("stone", 0).GetValue().c_str());
      rBuildingInfo.m_iBoards = atoi(rBuildingElement("boards", 0).GetValue().c_str());
      rBuildingInfo.m_iGold = atoi(rBuildingElement("gold", 0).GetValue().c_str());
      rBuildingInfo.m_iLines = atoi(rBuildingElement("lines", 0).GetValue().c_str());

      for(int i = 0; i < rBuildingInfo.m_iLines; ++i) {
        rBuildingInfo.m_vBuildingPosLines.push_back(
          atoi(rBuildingElement("buildingPosLines", 0)("value", i).GetValue().c_str())
        );
        rBuildingInfo.m_vDigPosLines.push_back(
          atoi(rBuildingElement("digPosLines", 0)("value", i).GetValue().c_str())
        );
        rBuildingInfo.m_vBlockPosLines.push_back(
          atoi(rBuildingElement("blockPosLines", 0)("value", i).GetValue().c_str())
        );
        rBuildingInfo.m_vRepealingPosLines.push_back(
          atoi(rBuildingElement("repealingPosLines", 0)("value", i).GetValue().c_str())
        );
      }
      ...
      if(BuildingTypeExIsShipyard(iBuildingIdx) | BuildingTypeExIsPort(iBuildingIdx)) {
        ...
        for(int i = 0; i < rBuildingInfo.m_iLines; ++i) {
          rBuildingInfo.m_vWaterPosLines.push_back(
            atoi(rBuildingElement("waterPosLines", 0)("value", i).GetValue().c_str())
          );
          rBuildingInfo.m_vWaterBlockPosLines.push_back(
            atoi(rBuildingElement("waterBlockPosLines", 0)("value", i).GetValue().c_str())
          );
          rBuildingInfo.m_vWaterFreePosLines.push_back(
            atoi(rBuildingElement("waterFreePosLines", 0)("value", i).GetValue().c_str())
          );
        }
      }
      ...
      for(int i = 0; i < rBuildingInfo.m_iBuilderNumber; ++i) {
        ...
        rBuildingInfo.m_vBuilder[i].m_iXOffset = atoi(rBuilderInfoElement("xOffset", 0).GetValue().c_str());
        rBuildingInfo.m_vBuilder[i].m_iYOffset = atoi(rBuilderInfoElement("yOffset", 0).GetValue().c_str());
        rBuildingInfo.m_vBuilder[i].m_iDirection = atoi(rBuilderInfoElement("dir", 0).GetValue().c_str());
      }
