...
CBuildingFlagsWalk::CBuildingFlagsWalk(int a2, int a3, int a4, int a5, std::vector<unsigned int> const &a6) : m_v0(a6) {
  this->m_iU0 = a2 - a4 - 1;
  this->m_iU1 = a3 - a5;
  CBuildingFlagsWalk::Init();
}
...
bool CBuildingFlagsWalk::NextPosition(void) {
  ...
  while(!this->m_pFirst) {
    if(this->m_iSteps >= this->m_v0.size()) {
      this->m_pFirst = 0;
      return 0;
    }
    this->m_iCurrentX = this->m_iU0;
    ++this->m_iCurrentY;
    this->m_pFirst = this->m_v0[m_iSteps];
    ++this->m_iSteps;
  }
  DWORD m_iCurrentX = this->m_iCurrentX;
