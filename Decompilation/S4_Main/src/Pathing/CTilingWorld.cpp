...
void __cdecl CTilingWorld::WorldInterfaceInit(int _iWorldWidthHeight, struct T_GFX_MAP_ELEMENT *_pGfxMapElements, unsigned char *_pFlagBitsLayer, unsigned char *_pFogLayer) {
  ...
  for(i = 0; i < 6; ++i) {
    ...
    //dword_42CF910[i] = v5; unused in the binary...
  }
  ...
  for(j = 0; j < 19; ++j) {
    CTilingWorld::m_iWorldSurroundingRelIndices[j] = _iWorldWidthHeight * CSpiralOffsets::DeltaY(j) + CSpiralOffsets::DeltaX(j);
  }
