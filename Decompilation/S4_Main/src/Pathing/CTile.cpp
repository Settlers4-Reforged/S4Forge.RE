...
int CTile::SectorId(void) const {
  return (unsigned __int16) this->m_iEcoSectorId;
}
...
int CTile::EcoSectorId(void) const {
  return HIWORD(this->m_iEcoSectorId);
}


// address=[0x133a160]
// Decompiled from BOOL __thiscall CTile::TilingType(CTile *this)
T_TILING_TYPE CTile::TilingType(void) const {
  return static_cast<T_TILING_TYPE>((this->m_iType & 0x8000000) != 0);
}
...
void CTile::SetEcoSectorId(int a2) {
  this->m_iEcoSectorId = static_cast<int16_t>(a2);
}
...
void CTile::SetSectorId(int a2) {
  this->m_iSectorId = static_cast<int16_t>(a2);
}
