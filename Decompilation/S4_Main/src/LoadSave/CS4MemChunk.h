...
class CS4MemChunk : public IS4Chunk {
  public:
    ...
    // address=[0x13ecd00]
    virtual void Load(void *_pData, unsigned int _uSize);

    // address=[0x13ece20]
    virtual unsigned int LoadUnsigned32(void);

    // address=[0x13ece40]
    virtual unsigned int LoadUnsigned32(unsigned int _uMin, unsigned int _uMax);

    // address=[0x13ecee0]
    virtual void LoadSignature(unsigned int _uSignature);

    // address=[0x13ecf40]
    virtual void Save(void const *_pTarget, unsigned int _uValue);

    // address=[0x13ecf60]
    virtual void SaveUnsigned32(unsigned int _uValue);

    // address=[0x13ecf80]
    virtual void SaveSignature(unsigned int _uSignature);
