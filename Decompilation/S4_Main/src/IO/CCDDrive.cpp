...
CCDDrive::CCDDrive(void) : ICDDrive()
{
  this->m_uDriveType = 0;
  memset(this->m_aU2, 0, sizeof(this->m_aU2));
}

const wchar_t *s_sCdDriveIdentifierNames[9] = {
    nullptr,
    nullptr,
    L"s4cd1.dat",
    L"s4x1cd1.dat",
...
enum T_S4_CDROM_TYPE CCDDrive::GetCDType(CHAR driveName)
{
  ...
  if (driveName >= 'a' && driveName <= 'z')
    driveName -= 32;
  if (driveName >= 'A' && driveName <= 'Z' && CCDDrive::GetDriveTypeA(driveName) == 5)
  {
    for (int i = 0; i < 9; ++i)
    {
      const wchar_t *v3 = s_sCdDriveIdentifierNames[i];
      WCHAR FileName[1024];
      if (v3)
      {
        wsprintfW(FileName, L"%c:\\%s", driveName, v3);
        if (CCDDrive::FindFile(FileName))
          return static_cast<T_S4_CDROM_TYPE>(i);
      }
    }
  }

  return S4_CDROM_TYPE_UNKNOWN;
}
...
char CCDDrive::GetCDDrive(enum T_S4_CDROM_TYPE a2)
{
  ...
  else
  {
    for (i = 65; i <= 90; ++i)
    {
      if (i != (char)v6)
      {
        v4 = this->GetCDType((unsigned __int8)i);
...
bool CCDDrive::GetCDPath(std::wstring &a2, wchar_t const *lpFileName, int a4)
{
  ...
  if ((a4 & 0x200000) != 0)
    a4 |= 0x10000u;
  if (!lpFileName)
    lpFileName = L""; //(LPCWSTR)&word_36BB364; seems to be empty with nothing writing to it
  v9 = 0;
  ...
  if (!v9)
  {
    if ((a4 & 0x400000) != 0)
    {
      ...
      for (i = 1; i < 9; ++i)
      {
        v8 = this->GetCDDrive(static_cast<T_S4_CDROM_TYPE>(i));
        if (v8)
        {
          if (!v12[v8])
          {
            v12[v8] = 1;
            wsprintfW(Source, L"%c:\\%s\\%s", v8, L"S4", lpFileName);
    ...
    {
      v9 = this->GetCDDrive(S4_CDROM_TYPE_UNKNOWN2);
    }
    else
    {
      for (j = 1; j < 9; ++j)
      {
        if ((a4 & (1 << j)) != 0)
        {
          v9 = this->GetCDDrive(static_cast<T_S4_CDROM_TYPE>(j));
          if (v9)
            break;
        }
      }
  ...
  if (v9)
  {
    if (v9 == 64)
    {
      if ((a4 & 0x20000) != 0)
        ...
        wsprintfW(String, L".\\%s", lpFileName);
    }
    else if ((a4 & 0x20000) != 0)
    {
      wsprintfW(String, L"%c:\\%s\\", v9, L"S4");
    }
    else
    {
      wsprintfW(String, L"%c:\\%s\\%s", v9, L"S4", lpFileName);
    }
