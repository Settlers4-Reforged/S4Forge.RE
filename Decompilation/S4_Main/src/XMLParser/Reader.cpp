...
namespace AdvXMLParser {
    ...
    // Parse the document
    {
        ...
        unique_ptr<Document> pDocument(Document::Parse(advself));
    ...
    {
        unique_ptr<Document> pDocument(new Document);
    ...
    void Document::ParseRootElement(Parser &parser) {
        // Get the root element
        unique_ptr<Element> pRoot(Element::Parse(parser, advself));
        if(NULL == pRoot.get())
    ...
    // [27] Misc ::= Comment | PI | S
    {
        ...
        unique_ptr<Comment> pComment(Comment::Parse(parser, advself));
        ...
        unique_ptr<Pi> pPI(Pi::Parse(parser, advself));
        if(NULL != pPI.get()) {
            Add(pPI.release());
            return (true);
        }
        ...
        while(c != cQuote) {
            switch(c) {
                ...
                case TEXT('&'): // Reference
                {
                    ...
                    unique_ptr<Reference> pReference(Reference::Parse(parser, Node::null));
    ...
    // Marker: '<'
    {
        ...
        unique_ptr<Element> pElem(ParseTagBegining(parser, bEmptyTag, parent));
        if(pElem.get() == NULL)
            return (NULL);
    ...
    // Marker: '<'
    {
        ...
        unique_ptr<Element> pElem(new Element(parent, strName));
        ...
        for(;;) {
            ...
            unique_ptr<Attribute> pAttribute(Attribute::Parse(parser, *pElem));
    ...
    void Element::ParseReference(Parser &parser) {
        ...
        unique_ptr<Reference> pReference(Reference::Parse(parser, advself));
        if(pReference.get() == NULL) // Failed ?
            parser.SyntaxError(ERROR_INVALID_REFERENCE);
    ...
    // Comment begins with   '<!--'
    {
        ...
        // Try to read a CDATA
        unique_ptr<CData> pCData(CData::Parse(parser, advself));
        if(NULL != pCData.get()) {
            // Add it to the element
            Add(pCData.release());
            return (false);
        ...
        unique_ptr<Element> pElement(Element::Parse(parser, advself));
    ...
    // Marker: Name
    {
        ...
        unique_ptr<Attribute> pAttrib(new Attribute(parent, strName));
    ...
    //
    {
        ...
        // A CharRef ?
        unique_ptr<Reference> pReference(CharRef::Parse(parser, parent));
        if(NULL != pReference.get())
            return (pReference.release());

        // Avoid to use unique_ptr::release here since it's not defined
        // by Microsoft STL.
    ...
    // Marker: '&'
    {
        ...
        unique_ptr<EntityRef> pRef(new EntityRef(parent, strReferenceName));
