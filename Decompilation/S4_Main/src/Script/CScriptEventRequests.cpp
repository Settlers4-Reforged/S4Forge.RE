...
void CScriptEventRequests::AddFunction(int _iScriptFuncId)
{
  ...
  if ((unsigned int)_iScriptFuncId >= 0x40 && BBSupportDbgReport(
                                                  2,
                                                  "Script\\GameScriptManager.cpp",
                                                  119,
                                                  "static_cast<unsigned int>(_iScriptFuncId) < CScriptManager::REGISTERED_FUNCTION_MAX") == 1)
  ...
  for (i = 0;; ++i)
  {
    if (i >= m_iSize)
      goto LABEL_10;
    if (this->m_aScriptFunc[i] == _iScriptFuncId)
      break;
    ...
    LABEL_10:
      ...
      if (v3 < 0)
        v3 = this->m_iSize++;
      if (this->m_iSize >= 0x40u && BBSupportDbgReport(
                                        2,
                                        "Script\\GameScriptManager.cpp",
                                        155,
...
void CScriptEventRequests::RemoveFunction(int a2)
{
  ...
  Function = this->FindFunction(a2);
  if (Function >= 0)
  {
    this->m_aScriptFunc[Function] = 255;
...
{
  if (_iIdx >= this->m_iSize && BBSupportDbgReport(
                                    2,
...
{
  this->m_iSize = 0;
  memset(this->m_aScriptFunc, 255, sizeof(this->m_aScriptFunc));
}
