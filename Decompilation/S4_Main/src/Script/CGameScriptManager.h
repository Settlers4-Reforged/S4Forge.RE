#ifndef CGAMESCRIPTMANAGER_H
#define CGAMESCRIPTMANAGER_H

#include "defines.h"

#include "LoadSave/IS4ChunkObject.h"
#include "CScriptEventRequests.h"

extern class CGameScriptManager *g_pScriptMgr;

class CGameScriptManager : public IS4ChunkObject {
public:
    // address=[0x131ec80]
    class CLua &  GetScriptEnv(void);

    // address=[0x1439ca0]
    void  SendGameEvent(int a2, int a3);

    // address=[0x1470c10]
    void  SendGameEvent(int a2, int a3, int a4, int a5, int a6);

    // address=[0x147e750]
    void  SendGameEvent(int a2);

    // address=[0x147e7c0]
    void  SendGameEvent(int a2, int a3, int a4);

    // address=[0x147e840]
    void  SendGameEvent(int a2, int a3, int a4, int a5);

    // address=[0x14875a0]
    void  SetVictoryConditionHook(void (__cdecl *a2)(void) );

    // address=[0x15feea0]
     CGameScriptManager(void);

    // address=[0x15fef60]
    virtual  ~CGameScriptManager(void);

    // address=[0x15fefe0]
    void  Update(unsigned int a2);

    // address=[0x15ff140]
    void  NewGame(char const * Src, unsigned int Size);

    // address=[0x15ff1f0]
    void  NewGameEx(char const * Src, unsigned int Size, wchar_t const * Source, bool a5);

    // address=[0x15ff6c0]
    virtual void  Load(class IS4Chunk & a2);

    // address=[0x15ff800]
    virtual void  Save(class IS4Chunk & a2);

    // address=[0x15ff890]
    void  StartScript(void);

    // address=[0x15ff960]
    void  CreateStartResources(int a2, int _iStartX, int _iStartY, int a5, int a6);

    // address=[0x15ffaf0]
    void  SetGlobal(char const * Str, int a3);

    // address=[0x16026c0]
    void  NewGame(class IScriptFile & a2);

    // address=[0x1602700]
    void  NewGame(void);

protected:
    // address=[0x15ffb30]
    bool  LoadMapScript(char const * Src, unsigned int Size);

    // address=[0x15ffc30]
    void  LoadState(class IS4Chunk & a2);

    // address=[0x15ffd30]
    void  SaveState(class IS4Chunk & a2);

    // address=[0x15ffe00]
    void  ExecuteScript(void);

    // address=[0x15ffe50]
    void  CallNewGame(void);

    // address=[0x15ffec0]
    void  CallRegisterFunctions(void);

    // address=[0x15fff50]
    void  InitScriptEnvironmentAndManager(void);

    // address=[0x1600110]
    void  UnrequestAllEvents(void);

    // address=[0x1600150]
    void  Done(void);

    // address=[0x1600230]
    void  SendGameEventCall(int _iEventType);

    // address=[0x1600370]
    void  SendGameEventCall(int _iEventType, int a3);

    // address=[0x16004b0]
    void  SendGameEventCall(int _iEventType, int a3, int a4);

    // address=[0x16005f0]
    void  SendGameEventCall(int _iEventType, int a3, int a4, int a5);

    // address=[0x1600730]
    void  SendGameEventCall(int _iEventType, int a3, int a4, int a5, int a6);

    // address=[0x1600880]
    void  GetSaveVarValues(double * a2, int iSize);

    // address=[0x1600970]
    void  SetSaveVarValues(double * a2, int iSize);

    // address=[0x1600a60]
    void  RequestEvent(int a2, int _iEventType);

    // address=[0x1600ab0]
    void  UnrequestEvent(int a2, int _iEventType);

    // address=[0x1600b00]
    static void __cdecl LuaRequestEvent(void);

    // address=[0x1600bf0]
    static void __cdecl LuaRequestEvents(void);

    // address=[0x1600d20]
    static void __cdecl LuaUnrequestEvent(void);

    // address=[0x1600e10]
    static void __cdecl LuaUnrequestEvents(void);

    // address=[0x1600f40]
    static void __cdecl LuaErrorToTrace(void);

    // address=[0x1602590]
    class CScriptManager &  GetScriptManager(void);

private:
    // address=[0x462b824]
    static class CGameScriptManager * m_pGameScriptManager;

    // Type information members
public:
    int m_uGameScriptState;
    int m_uStartOfGameState;
    CLua * m_pScriptEnv;
    CScriptManager * m_pScriptManager;
    void * m_pMapScriptData;
    int m_uMapScriptSize;
    CScriptEventRequests m_aScriptEventRequests[23];
    void (__cdecl *m_pVictoryConditionHook)();

};


#endif // CGAMESCRIPTMANAGER_H
