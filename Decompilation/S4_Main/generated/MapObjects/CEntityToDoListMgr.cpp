  ...
  for ( m = 1; m < 66; ++m )
  {
    for ( i = 0; i < 5; ++i )
    {
      ...
      else
      {
        ...
        std::list<CEntityTask>::clear(this->m_vSettlerJobsList[i][iJobListAfterSettlerIter]);
        iFrameCount = CGfxManager::GetSettlerJobFrameCount(g_pGfxManager, i, iFirstJobId, 2u);
        v2 = CEntityTask::CEntityTask(&v35, 10, iFirstJobId, 0, 0, -1, iFrameCount, 1, 1, 1, 0, 0, 0, 0);
        std::list<CEntityTask>::push_back(this->m_vSettlerJobsList[i][iJobListAfterSettlerIter], (int)v2);
        v3 = CEntityTask::CEntityTask(&v34, 24, iFirstJobId, 0, 0, -1, iFrameCount, 1, 0, 1, 0, 0, 0, 0);
        std::list<CEntityTask>::push_back(this->m_vSettlerJobsList[i][iJobListAfterSettlerIter], (int)v3);
      }
    }
...
void __cdecl CEntityToDoListMgr::GetJobSoundInfo(int a1, struct SJobSoundInfo & a2) {
  ...
  a2->m_iId = CEntityToDoListMgr::m_vJobSoundInfo[a1].m_iId;
  a2->m_iFrame = CEntityToDoListMgr::m_vJobSoundInfo[a1].m_iFrame;
  a2->m_iRandom = CEntityToDoListMgr::m_vJobSoundInfo[a1].m_iRandom;
}
