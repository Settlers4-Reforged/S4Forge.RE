...
public:
    ...
    // address=[0x1588900]
    static class CSoldierRole * __cdecl Load(std::istream &);
...
protected:
    ...
    // address=[0x158fe10]
    void  SoldierMagicIdleWalk(class CSettler *);
    ...
    // address=[0x1590f40]
    void  ComeToWork(class CSettler *,int);

    // address=[0x1591100]
    bool  NewDestinationEx(class CSettler *,int,int,int);

    // address=[0x1591450]
    void  EvaluateWalkAndRegister(class IMovingEntity &,int);

    // address=[0x1592280]
    void  SoldierWarriorLogicUpdate(class CSettler *);
