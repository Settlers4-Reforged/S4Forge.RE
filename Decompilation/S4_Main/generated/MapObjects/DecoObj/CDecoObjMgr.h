...
public:
    ...
    // address=[0x15428b0]
    int  AddDecoObj(int _iX, int _iY, int _iDecoType, int _iAmount, enum CDecoObjMgr::TGROWN_STATE _iGrownState);

    // address=[0x1542970]
    int  AddReef(int _iX, int _iY, int _iDecoType, int _iAmount, enum CDecoObjMgr::TGROWN_STATE _iGrownState);

    // address=[0x1542a80]
    int  AddDecoObjWithoutFlags(int _iX, int _iY, int _iType, int _iAmount, enum CDecoObjMgr::TGROWN_STATE _iGrownState);

    // address=[0x1542b50]
    int  AddLoadedDecoObj(int _iX, int _iY, int _iType, int _iAmount, int _iSlot, bool a7, std::istream * _pInStream);
...
private:
    // address=[0x1544b00]
    int  Create(int _iX, int _iY, int _iDecoType, int _iAmount, enum CDecoObjMgr::TGROWN_STATE _iGrownState, int _iSlot, std::istream * _pInStream);
...
public:
    ...
    _DWORD m_iXmlVersion;
    bool m_bHasLoaded;
    CDecoObjMgr::Object[291] m_aObjects;
    CDecoObjMgr::Ring[291] m_aBuildingRings;
