...
void  CPileObserverList::Attach(enum T_OBSERVER_TARGET _tTargetType, int _iTargetId, int _iObserverId, int _iDeltaAmount) {
  ...
  if ( _iObserverId > 0xFFFF && BBSupportDbgReport(2, "MapObjects\\Pile\\PileObserverList.cpp", 30, "_iObserverId <= 0xFFFF") == 1 )
  {
    __debugbreak();
  }
  if ( (_iDeltaAmount < -8 || _iDeltaAmount > 8) && BBSupportDbgReport(2, "MapObjects\\Pile\\PileObserverList.cpp", 32, "(_iDeltaAmount >= -8) && (_iDeltaAmount <= 8)") == 1 )
  {
    __debugbreak();
  }
  ...
  if ( CPileObserverList::GetIndex(this, _iObserverId) >= 0 && BBSupportDbgReport(2, "MapObjects\\Pile\\PileObserverList.cpp", 35, "GetIndex(_iObserverId) < 0") == 1 )
  {
    __debugbreak();
  }
  if ( this->m_uNumberOfObservers >= 8u && BBSupportDbgReport(2, "MapObjects\\Pile\\PileObserverList.cpp", 37, "m_uNumberOfObservers < PILE_OBSERVERS_MAX") == 1 )
  {
    __debugbreak();
  }
...
int  CPileObserverList::Detach(int _iObserverId) {
  ...
  if ( iIndex < 0 )
  {
    if ( BBSupportDbgReportF(1, "MapObjects\\Pile\\PileObserverList.cpp", 98, "CPileObserverList::Detach(): Observer %i not in list!", _iObserverId) == 1 )
    {
      __debugbreak();
    }
  }
  else
  {
    ...
    if ( iIndex > iNewNumberOfObservers && BBSupportDbgReport(2, "MapObjects\\Pile\\PileObserverList.cpp", 75, "iIndex <= iNewNumberOfObservers") == 1 )
    {
      __debugbreak();
    }
...
void  CPileObserverList::NotifyAllObservers(class CEntityEvent const & a2)const {
        ...
        ++i )
  {
    ...
    else if ( BBSupportDbgReportF(1, "MapObjects\\Pile\\PileObserverList.cpp", 125, "CPileObserverList::NotifyAllObservers(): Invalid observer %i in list!", iObserverId) == 1 )
    {
      __debugbreak();
    }
  }
...
// Decompiled from CPileObserverList *__thiscall CPileObserverList::CPileObserverList(CPileObserverList *this, int _rStream)
 CPileObserverList::CPileObserverList(std::istream & _rStream) {
  ...
  if ( this->m_uNumberOfObservers > 8u && BBSupportDbgReport(2, "MapObjects\\Pile\\PileObserverList.cpp", 215, "m_uNumberOfObservers <= PILE_OBSERVERS_MAX") == 1 )
  {
    __debugbreak();
  }
  for ( i = 0;
