...
void CAnyWherePileRole::Init(class CPile *_pPile) {
    ...
    _pPile->ClearAllQueuedEvents(_pPile);
    CPile::SetRoleType(_pPile, 3u);
    CPile::SetOfferFlag(_pPile, 1u);
    IEntity::SetFlagBits(_pPile, ENTITY_FLAG_Visible);
    if(_pPile->Amount(_pPile)) {
        CPile::OfferCompletePileIfPossible(_pPile, 0);
        IAnimatedEntity::RegisterForLogicUpdate(_pPile, 31);
    } else {
