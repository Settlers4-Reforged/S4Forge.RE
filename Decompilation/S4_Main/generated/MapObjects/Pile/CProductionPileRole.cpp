...
class CPersistence *__cdecl CProductionPileRole::New(std::istream &_rStream) {
    ...
    if(C)
        CProductionPileRole::CProductionPileRole(C, (int)_rStream);
}
...
void CProductionPileRole::Init(class CPile *a2) {
    ...
    if(!CWorldManager::FlagBits(_pPile, 8u) && BBSupportDbgReport(
                                                   2,
    ...
    if(CWorldManager::EcoSectorId(v3) <= 0 && BBSupportDbgReport(
                                                  2,
                                                  "MapObjects\\Pile\\ProductionPileRole.cpp",
                                                  95,
...
void CProductionPileRole::ReassessDistance(class CPile *_pPile, int &a3) {
