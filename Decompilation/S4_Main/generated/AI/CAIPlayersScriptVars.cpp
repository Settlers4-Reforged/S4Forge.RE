...
class CAIPlayerScriptVars const &  CAIPlayersScriptVars::operator[](int _iPlayerId) {
  ...
  if ( _iPlayerId >= 9 && BBSupportDbgReport(2, "d:\\projects\\tshe\\purplelamp\\s4\\source\\s4_main\\ai\\AI_Script.h", 173, "static_cast<unsigned int>(_iPlayerId) < PLAYER_MAX") == 1 )
  {
    __debugbreak();
  }
...
int  CAIPlayersScriptVars::GetValue(int _iPlayerId, enum T_AI_PLAYER_SCRIPT_VAR_INDEX a3) {
  ...
  if ( (_iPlayerId >= 9 || _iPlayerId < 0) && BBSupportDbgReport(2, "d:\\projects\\tshe\\purplelamp\\s4\\source\\s4_main\\ai\\AI_Script.h", 229, "(_iPlayerId >= 0) & (_iPlayerId < PLAYER_MAX)") == 1 )
  {
    __debugbreak();
  }
