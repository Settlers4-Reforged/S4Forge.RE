...
void  CDebugGridTest::Test(void) {
        ...
        ++i )
  {
    ...
    if ( !SPoint::operator==(&g_sSurroundingHexPoints[i + 1], &g_sNeighborPoints[i]) && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1434, "g_sSurroundingHexPoints[1 + i] == g_sNeighborPoints[i]") == 1 )
    {
      __debugbreak();
    }
    v20 = SPoint::Y(&g_sNeighborPoints[i]);
    v0 = SPoint::X(&g_sNeighborPoints[i]);
    if ( Grid::Direction(v0, v20) != i && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1435, "Direction(g_sNeighborPoints[i].X(), g_sNeighborPoints[i].Y()) == i") == 1 )
    {
      __debugbreak();
    }
  }
  SPoint::Zero(&sZero);
  if ( !SPoint::operator==(g_sSurroundingHexPoints, &sZero) && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1446, "g_sSurroundingHexPoints[0] == sZero") == 1 )
  {
    __debugbreak();
  }
        ...
        ++j )
  {
    ...
    if ( j )
    {
      ...
      if ( j < v2 && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1482, "i >= SurroundingHexPointsCount(iRadius - 1)") == 1 )
      {
        __debugbreak();
      }
    ...
    if ( v13 != iRadius && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1503, "Distance2a(0, 0, iDX, iDY) == iRadius") == 1 )
    {
      __debugbreak();
    }
    v14 = Y16X16::Distance2a(iDX, iDY, 0, 0);
    if ( v14 != iRadius && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1504, "Distance2a(iDX, iDY, 0, 0) == iRadius") == 1 )
    {
      __debugbreak();
    }
    v15 = Y16X16::Distance2b(0, 0, iDX, iDY);
    if ( v15 != iRadius && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1505, "Distance2b(0, 0, iDX, iDY) == iRadius") == 1 )
    {
      __debugbreak();
    }
    v16 = Y16X16::Distance2b(iDX, iDY, 0, 0);
    if ( v16 != iRadius && BBSupportDbgReport(2, "Pathing\\Grid.cpp", 1506, "Distance2b(iDX, iDY, 0, 0) == iRadius") == 1 )
    {
      __debugbreak();
    }
  }
