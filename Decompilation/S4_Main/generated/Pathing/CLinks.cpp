...
int __cdecl CLinks::PushLinksUndef(int a1) {
  ...
  CLink *v1; // eax
  int iSizeEx; // [esp+0h] [ebp-8h]
  int iUnusedLinksId; // [esp+4h] [ebp-4h]
...
// address=[0x15e70b0]
// Decompiled from BOOL __cdecl CLinks::IsValidRealLinkList(const struct CLink *a1)
bool __cdecl CLinks::IsValidRealLinkList(class CLinkList const & a1) {
  
  return a1 <= &CLinks::m_cLinks[262141] && a1 >= &CLinks::m_cLinks[2];
}
...
int __cdecl CLinks::SearchForUnusedLinkEntries(int _iSizeEx) {
  ...
  if ( CLinks::m_iMinFreeLinksId < CLinks::m_iMaxUsedLinksId )
  {
    ...
    v12 = _iSizeEx + CLinks::m_iMinFreeLinksId;
    while ( 1 )
    {
      v10 = CLinks::Link(v15);
