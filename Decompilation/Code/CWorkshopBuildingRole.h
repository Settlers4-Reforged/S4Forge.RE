...
public:
    ...
    // address=[0x14fdd30]
    static class CWorkshopBuildingRole * __cdecl Load(std::istream &);
    ...
    // address=[0x152dc20]
    bool  HaveSpecialOrder(int)const;
    ...
    // address=[0x152dde0]
    void  FillAddVehicleSideBar(class CAddVehicleBarInfo *,bool);

    // address=[0x152dfd0]
    bool  CancelCurrentProduction(void);

    // address=[0x152e100]
     CWorkshopBuildingRole(std::istream &);
...
private:
    ...
    // address=[0x152e5c0]
    void  GetPositionForNewVehicle(class CBuilding *,int &,int &);

    // address=[0x152e610]
    int  GetDirectionForNewVehicle(class CBuilding *);

    // address=[0x152e6c0]
    bool  IsSpaceForVehicleAvailable(class CBuilding *,int);
